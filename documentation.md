...
4. **CI** runs flake8 and offline pytest on every PR to `main` (uses committed snapshots).
...
tests/test_pipeline.py             # pytest suite (committed snapshots)
...
Requires network access for `python main.py fetch`; pytest uses committed snapshots:
...
| **Steps** | `pip install` → flake8 → `pytest` (committed snapshots) |
| **Network** | Not required for pytest (live fetch only when snapshots are missing) |
...
- All **7** pytest checks passing on committed live-data snapshots
